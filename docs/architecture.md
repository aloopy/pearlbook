...
Medical knowledge agent
   |-- local file search/read/write --> Obsidian vault
...
The **vault is the durable clinical memory**. The agent's chat history, model context, and platform memory can improve continuity, but none should be the only copy of a clinical note.
